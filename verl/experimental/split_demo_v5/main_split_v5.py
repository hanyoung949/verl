"""split demo v5 主入口 — 3-stage pipeline + Qwen3.5 MTP rollout。
...
@hydra.main(config_path="config", config_name="split_demo_v5", version_base=None)
def main(config: DictConfig):
    ...
    engine.trust_remote_code = bool(config.model.get("trust_remote_code", False))
    mtp_cfg = config.model.get("mtp", {})
    engine.mtp_enable = bool(mtp_cfg.get("enable", False))
    engine.mtp_enable_train = bool(mtp_cfg.get("enable_train", False))
    engine.mtp_enable_rollout = bool(mtp_cfg.get("enable_rollout", False))
    engine.mtp_num_speculative_tokens = int(mtp_cfg.get("num_speculative_tokens", 1))
    engine.initialize()
