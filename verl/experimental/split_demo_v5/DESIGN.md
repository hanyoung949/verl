# Split Demo v5 — 3-Stage Pipeline 设计文档

**目标**：在 3 张 GPU 上验证 Head–Middle–Tail 三层拆分的 GRPO 训练通路，通信协议稳定，支持 Tail 本地采样与 Qwen3.5 MTP rollout。  
**范围**：`verl/experimental/split_demo_v5/`。  
**验证平台**：单机 3×GPU，`torchrun --nproc_per_node=3`。  
**与 v4 的关系**：v5 从 v4 复制而来，仅新增 MTP rollout；不改 v4，也不引入多 GPU per stage。
...
v4 已经把 middle layers 独立成一个 **MiddleStage**，形成真正的 3-stage 流水线；v5 在这个结构上新增 MTP rollout：
...
```
torchrun --nproc_per_node=3 -m verl.experimental.split_demo_v5.main_split_v5
...
Tail → Head:    next_token [B,L] + log_prob [B,L] + token_mask [B,L]
```
...
- 非 MTP 路径只回传 `next_token [B,1]` + `log_prob [B,1]`
...
### A4. Qwen3.5 MTP speculative rollout
- 配置默认模型为 `Qwen/Qwen3.5-2B`，该模型的 `text_config.mtp_num_hidden_layers=1`
- Tail 初始化时从 checkpoint safetensors 显式读取 `mtp.*` 权重，构建 MTP draft head
- `_tail_sample()` 先用 MTP draft head 递归生成 `num_speculative_tokens` 个 draft token
- Head 收到 draft 后，用原有 Head→Middle→Tail pipeline 对 `prefix + draft_tokens` 做一次 target verification
- Tail 按 speculative accept/reject 返回最多 `num_speculative_tokens + 1` 个 token，并附带 `token_mask`
- `SimplePipelineRollout` 一次追加多个有效 token；GRPO trainer 仍消费最终 `response_ids / response_mask / old_log_probs`
- 默认 `num_speculative_tokens=4`
...
1. **Tail 采样未实现 top-p**：`_tail_sample()` 只做 temperature softmax + multinomial，`top_p` 配置被忽略。当前默认 `top_p=1.0` 不影响正确性；若需支持，需在 Head→Tail 通信中增加 top_p 参数传递。
2. **NCCL P2P 无超时**：当前依赖默认 60s timeout，未配置化（B 阶段考虑）。
3. **MTP 仅用于 rollout**：`model.mtp.enable_train=false`，不加入辅助 MTP loss；这是为了保持 v4 的 GRPO 训练通路最小变更。

> ✅ 已修复：Head checkpoint 完整性（C3）、rank 硬编码（C1）、Middle flag 校验（B3）。
...
CUDA_VISIBLE_DEVICES=5,6,7 torchrun --nproc_per_node=3 \
  -m verl.experimental.split_demo_v5.main_split_v5
```
