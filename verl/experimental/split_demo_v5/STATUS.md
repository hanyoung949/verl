...
| Phase D | Qwen3.5 MTP rollout：加载 `mtp.*` 权重 + multi-token verification | 代码完成 | 静态编译通过，待 3GPU smoke |
...
```
model: Qwen/Qwen3.5-2B (24 层, hidden=2048, vocab=248320, MTP=1)
split: 4:16:4 (front=4, middle=16, tail=4)
topology: head=[0], middle=[1], tail=[2]
mtp: enable=true, enable_rollout=true, enable_train=false, num_speculative_tokens=4
