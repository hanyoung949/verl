...
- `main_split_v5.py` Head 接收 logits：shape tensor 从 `torch.zeros(2)` 改为 `torch.zeros(3)`
- `main_split_v5.py` `_tail_forward_logits()` 发送 shape：增加 `logits.shape[2]`
...
- `main_split_v5.py` 从 500+ 行精简到 ~80 行，只负责组件组装
...
- 新增 `rollout/simple_pipeline_rollout.py`：`SimplePipelineRollout` 从 `main_split_v5.py` 移入
