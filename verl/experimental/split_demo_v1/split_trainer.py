...
class SplitGRPOTrainer:
    ...
    def _build_uid_array(self, prompt_batch_size: int) -> np.ndarray:
        ...
        同一个 prompt 生成出来的 G 条 response 要共享同一个 uid，这样
        `compute_advantage(..., adv_estimator=GRPO)` 才能在组内做 relative advantage。
        """
    ...
    def fit(self, samples: list[dict[str, Any]]) -> None:
        ...
        为了让第一版更容易调通，这里采取“随机采样 prompt batch”的方式，而不是
        一上来就引入 DataLoader / 多进程数据管线。
        """
        ...
        for global_step in range(1, total_steps + 1):
            ...
            # rollout 阶段不需要梯度。
            # 如果不显式包在 no_grad() 下，LoRA 参数仍会参与 autograd 图构建，
            # 白白占用一整段生成过程的显存。
            with torch.no_grad():
            ...
            # 这里不再依赖 ray_trainer.compute_advantage。
            # 原因：
            # 1. 它会额外引入一大串主线 trainer / checkpoint / fsdp 依赖；
            # 2. 当前 demo 只需要 GRPO outcome advantage，本地直接调用 core_algos 更轻、更稳。
    ...
    def _reset_peak_memory_stats(self) -> None:
        """在每个训练 step 开始前重置峰值显存统计。

        这样后面打印出来的 peak 数值更接近“本 step 内实际冲到多高”，
        比只看 `nvidia-smi` 的瞬时快照更有信息量。
        """
        ...
        seen: set[int] = set()
        for device_name in [getattr(self.model, "primary_device", None), getattr(self.model, "middle_device", None)]:
    ...
    def _format_memory_stats(self) -> str:
        ...
        seen: set[int] = set()
        labels = [
            ("gpu0", getattr(self.model, "primary_device", None)),
            ("gpu1", getattr(self.model, "middle_device", None)),
        ]

        for label, device_name in labels:
            if not device_name or not str(device_name).startswith("cuda:"):
                continue
            index = int(str(device_name).split(":")[1])
            if index in seen:
            ...
            parts.append(
                f"{label}_alloc={allocated:.2f}GB "
                f"{label}_reserv={reserved:.2f}GB "
                f"{label}_peak={peak:.2f}GB"
            )
    ...
    def _format_transport_stats(self) -> str:
        ...
        - position / mask 附带体积
        - 中段输出搬回主卡的体积
        - 当前 step 一共调用了多少次 full forward / prefill / decode_step
        """
    ...
    def _save_training_artifacts(self, global_step: int) -> None:
        """保存最小 adapter / checkpoint 产物。

        保存策略：
        - `save_dir/global_step_{N}`：保留本次训练结果
        - `save_dir/latest`：固定位置，始终指向最近一次结果
        """
